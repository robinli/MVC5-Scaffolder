...
namespace WebApp.Controllers
{
    public class OrderDetailsController : Controller
    {
        ...
        [HttpGet]
        public ActionResult GetDataByOrderId(int orderid=0,int page = 1, int rows = 10, string sort = "Id", string order = "desc", string filterRules = "")
        {
            var filters = JsonConvert.DeserializeObject<IEnumerable<filterRule>>(filterRules);
            int totalCount = 0;
            //int pagenum = offset / limit +1;

            var orderdetails = _orderDetailService.Query(new OrderDetailQuery().ByOrderIdWithfilter(orderid,filters)).Include(o => o.Order).Include(o => o.Product).OrderBy(n => n.OrderBy(sort, order)).SelectPage(page, rows, out totalCount);
