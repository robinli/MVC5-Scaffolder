                        ...
                        this.each(function() {
                            this.setSelectionRange
                                ...
                                (range = this.createTextRange(),
                                    range.collapse(!0), range.moveEnd("character", end), range.moveStart("character",
                                        begin),
                                    range.select());
                        }))
                ...
                firstNonMaskPos = null, $.each(mask.split(""),
                    function(i, c) {
                        "?" == c
                            ...
                            ? (tests.push(new RegExp(defs[c])),
                                null === firstNonMaskPos && (firstNonMaskPos = tests.length - 1), partialPosition > i &&
                    ...
                    function shiftR(pos) {
                        ...
                        for (i = pos, c = getPlaceholder(pos); len > i; i++)
                            if (tests[i]) {
                                if (j = seekNext(i), t = buffer[i], buffer[i] =
                                    c, !(len > j && tests[j].test(t))) break;
                                c = t;
                            }
                    }
                            ...
                            oldVal = input.val(), 8 === k || 46 === k || iPhone && 127 === k
                                ? (pos = input.caret(),
                                    ...
                                    (begin = 46 !== k ? seekPrev(begin) : end = seekNext(begin - 1),
                                        end = 46 === k ? seekNext(end) : end), clearBuffer(begin, end), shiftL(begin,
                                        end - 1),
                                    e.preventDefault())
                                : 13 === k
                    ...
                    input.data($.mask.dataName,
                            ...
                            .on("keypress.mask", keypressEvent).on("input.mask paste.mask",
                                function() {
                                    input.prop("readonly") ||
                                        setTimeout(function() {
                                                var pos = checkVal(!0);
